import { resolveReturnUrl } from "@/utils/merchants";
...
export default function DetailScreen() {
  const { colors, returns, setReturnOption, markReturned, removeReturn } =
    useApp();
  ...
  if (!item)
    return (
      <View style={[styles.center, { backgroundColor: colors.background }]}>
        ...
        <Text style={[styles.missingCopy, { color: colors.mutedForeground }]}>
          Puede que ya esté en tu historial.
        </Text>
  ...
  const returnUrl = resolveReturnUrl(item.store, item.returnsUrl);
  ...
  const dueCopy =
    days < 0
      ? "El plazo indicado ya ha pasado"
      : days === 0
        ? "Último día para devolverlo"
        : days === 1
          ? "Mañana es el último día"
          : `Tienes ${days} días para decidir`;

  const openReturns = async () => {
    if (!returnUrl) {
      router.push({ pathname: "/upload", params: { edit: item.id } });
      return;
    }
    try {
      ...
      Alert.alert(
        "No se pudo abrir",
        "Comprueba el enlace de devolución en los datos de la compra.",
      );
    }
  };
  ...
  const finishReturn = (outcome: "in_progress" | "refunded") => {
    const title =
      outcome === "refunded"
        ? "¿Confirmas que recibiste el reembolso?"
        : "¿Ya enviaste o entregaste el artículo?";
    const body =
      outcome === "refunded"
        ? "La compra se guardará en el historial como reembolsada."
        : "La compra pasará al historial para que puedas seguir el reembolso.";
  ...
  const deleteItem = () =>
    Alert.alert(
      "Eliminar compra",
      "Se eliminará de tu lista y no aparecerá en el historial.",
      [
        { text: "Cancelar", style: "cancel" },
        {
          text: "Eliminar",
        ...
        <View style={styles.content}>
          ...
          >
            <View style={styles.heroTop}>
              <View
                style={[styles.storeMark, { backgroundColor: item.accent }]}
              >
              ...
              <View style={styles.heroCopy}>
                ...
                <Text
                  style={[styles.itemStore, { color: colors.mutedForeground }]}
                >
                  {item.store}
                </Text>
              </View>
          ...
          <View
            style={[
              styles.deadlineCard,
              { backgroundColor: urgent ? colors.urgentSoft : colors.accent },
            ]}
          >
            <View
              style={[styles.deadlineIcon, { backgroundColor: colors.card }]}
            >
              <Feather name="clock" size={18} color={tone} />
            </View>
            <View style={styles.deadlineCopy}>
              <Text
                style={[
                  styles.deadlineEyebrow,
                  { color: colors.mutedForeground },
                ]}
              >
                FECHA LÍMITE INDICADA
              </Text>
              <Text style={[styles.deadlineDate, { color: colors.foreground }]}>
                {formatDate(item.deadline)}
              </Text>
              <Text style={[styles.deadlineHint, { color: tone }]}>
                {dueCopy}
              </Text>
            </View>
            <View style={styles.countdown}>
              <Text style={[styles.countdownNumber, { color: tone }]}>
                {days < 0 ? "!" : days}
              </Text>
              <Text
                style={[
          ...
          >
            ...
            >
              Fecha guardada por ti. Confirma las condiciones exactas en tu
              pedido o con la tienda.
            </Text>
          </View>
          ...
          <View style={styles.optionsRow}>
            {["Reembolso", "Vale", "Cambio"].map((label) => {
              const selected = item.returnOption === label;
              return (
                <Pressable
                  key={label}
                  ...
                  onPress={() => {
                    setReturnOption(item.id, selected ? undefined : label);
          ...
          >
            ...
            >
              {returnUrl ? "Ir a devoluciones" : "Añadir enlace de esta tienda"}
            </Text>
            <Feather
              name="external-link"
              size={16}
          ...
          <Pressable
            onPress={() => finishReturn("in_progress")}
            style={[
          ...
          >
            ...
            <Text style={[styles.secondaryText, { color: colors.foreground }]}>
              Ya inicié la devolución
            </Text>
          </Pressable>
          <Pressable
            onPress={() => finishReturn("refunded")}
            style={styles.refundButton}
          >
            <Feather name="check-circle" size={15} color={colors.success} />
            <Text style={[styles.refundText, { color: colors.success }]}>
              Ya recibí el reembolso
            </Text>
          </Pressable>
          <Pressable onPress={deleteItem} style={styles.deleteButton}>
            <Text
              style={[styles.deleteText, { color: colors.mutedForeground }]}
            >
              Eliminar compra
            </Text>
          </Pressable>
...
const styles = StyleSheet.create({
  ...
  deadlineEyebrow: { fontSize: 11, fontWeight: "700", letterSpacing: 0.8 },
  deadlineDate: { fontSize: 17, fontWeight: "700" },
  deadlineHint: { fontSize: 14, fontWeight: "600" },
  ...
  countdownLabel: { fontSize: 11, fontWeight: "600" },
  sourceNote: {
    borderRadius: 13,
    borderWidth: 1,
    padding: 11,
  ...
  sourceText: { flex: 1, fontSize: 13, lineHeight: 21 },
  datesRow: {
    flexDirection: "row",
    alignItems: "center",
    marginTop: 19,
  ...
  refundText: { fontSize: 14, fontWeight: "600" },
